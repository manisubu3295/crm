...
var import_zod, import_dotenv, envSchema, parsed, env;
var init_env = __esm({
  "server/multitenant/env.ts"() {
    "use strict";
    ...
    envSchema = import_zod.z.object({
      ...
      // Multi-tenancy
      TENANT_REGISTRY_PATH: import_zod.z.string(),
      TENANT_ADMIN_KEY: import_zod.z.string().optional(),
      DEMO_TENANT_ID: import_zod.z.string().default("demo"),
      DEMO_TENANT_FALLBACK: import_zod.z.coerce.boolean().default(false),
      TENANT_DB_POOL_MAX: import_zod.z.coerce.number().default(10),
      TENANT_DB_IDLE_MS: import_zod.z.coerce.number().default(3e4),
      TENANT_DB_CONN_MS: import_zod.z.coerce.number().default(5e3),
      // Security
      JWT_SECRET: import_zod.z.string().min(32, "JWT_SECRET must be at least 32 characters"),
      CORS_ORIGINS: import_zod.z.string().default("http://localhost:5173"),
      RATE_LIMIT_WINDOW_MS: import_zod.z.coerce.number().default(9e5),
      RATE_LIMIT_MAX: import_zod.z.coerce.number().default(200),
      // Redis
      REDIS_URL: import_zod.z.string().default("redis://localhost:6379"),
      // Meta / WhatsApp
      META_APP_SECRET: import_zod.z.string().optional(),
      META_VERIFY_TOKEN: import_zod.z.string().optional(),
      META_ACCESS_TOKEN: import_zod.z.string().optional(),
      META_PHONE_NUMBER_ID: import_zod.z.string().optional(),
      META_API_VERSION: import_zod.z.string().default("v25.0"),
      // SendGrid
      SENDGRID_API_KEY: import_zod.z.string().optional(),
      EMAIL_FROM: import_zod.z.string().default("noreply@crm.local"),
      EMAIL_FROM_NAME: import_zod.z.string().default("CRM"),
      // Msg91
      MSG91_AUTH_KEY: import_zod.z.string().optional(),
      MSG91_SENDER_ID: import_zod.z.string().default("TRACRM"),
      MSG91_ROUTE: import_zod.z.string().default("4"),
      MSG91_OTP_TEMPLATE_ID: import_zod.z.string().optional(),
      // Exotel
      EXOTEL_API_KEY: import_zod.z.string().optional(),
      EXOTEL_API_TOKEN: import_zod.z.string().optional(),
      EXOTEL_SID: import_zod.z.string().optional(),
      EXOTEL_CALLER_ID: import_zod.z.string().optional(),
      EXOTEL_SUBDOMAIN: import_zod.z.string().default("api.exotel.com"),
      // Reports
      PUPPETEER_EXECUTABLE_PATH: import_zod.z.string().optional(),
      REPORT_TMP_DIR: import_zod.z.string().default("/var/tmp/crm-reports"),
      REPORT_PDF_CONCURRENCY: import_zod.z.coerce.number().default(2),
      TEMPLATE_CACHE_TTL_MS: import_zod.z.coerce.number().default(6e5),
      // Web Push (VAPID)
      VAPID_PUBLIC_KEY: import_zod.z.string().optional(),
      VAPID_PRIVATE_KEY: import_zod.z.string().optional(),
      VAPID_SUBJECT: import_zod.z.string().default("mailto:admin@crm.local"),
      // Razorpay
      RAZORPAY_KEY_ID: import_zod.z.string().optional(),
      RAZORPAY_KEY_SECRET: import_zod.z.string().optional(),
      RAZORPAY_WEBHOOK_SECRET: import_zod.z.string().optional(),
      // Migrations
      MIGRATION_TRANSACTION_ENABLED: import_zod.z.coerce.boolean().default(true),
      // Logging
...
var import_pino, logger, logger_default;
...
function loadRegistry() {
  try {
    const raw = import_fs.default.readFileSync(env.TENANT_REGISTRY_PATH, "utf-8");
...
function registerTenant(id, config2) {
  ...
  import_fs.default.writeFileSync(env.TENANT_REGISTRY_PATH, JSON.stringify(_registry, null, 2));
  logger_default.info({ tenantId: id }, "Tenant registered and persisted");
}
var import_fs, _registry;
...
async function probeRedis() {
  ...
  return new Promise((resolve) => {
    const socket = (0, import_net.createConnection)({ host, port });
...
function getRedisConnection() {
  if (!_connection) {
    _connection = new import_ioredis.Redis(env.REDIS_URL, {
      maxRetriesPerRequest: null
    });
  }
  return _connection;
}
var import_ioredis, import_net, _connection, nullQueue;
...
function getAutomationQueue() {
  if (!_queue) {
    _queue = new import_bullmq.Queue("crm-automation", {
...
var import_bullmq, _redisAvailable, _queue, automationQueue;
...
var import_axios, BASE, whatsappService;
...
var import_exceljs, import_handlebars, import_puppeteer, import_fs3, import_path2, activePdfs, reportService, defaultCss;
var init_reportService = __esm({
  "server/multitenant/services/reportService.ts"() {
    "use strict";
    import_exceljs = __toESM(require("exceljs"), 1);
    ...
    reportService = {
      ...
      async exportPdf(db, tenantId, type, opts) {
        return withPdfSlot(async () => {
          ...
          const compiled = import_handlebars.default.compile(htmlTemplate);
          const html = `<html><head><style>${css}</style></head><body>${compiled(data)}</body></html>`;
          const tmpFile = import_path2.default.join(env.REPORT_TMP_DIR, `report-${tenantId}-${Date.now()}.html`);
          import_fs3.default.mkdirSync(env.REPORT_TMP_DIR, { recursive: true });
          import_fs3.default.writeFileSync(tmpFile, html);
          let browser;
          try {
            ...
            await browser?.close();
            import_fs3.default.unlinkSync(tmpFile);
          }
        });
      }
    };
...
function getFollowUpQueue() {
  if (!_queue2) {
    _queue2 = new import_bullmq2.Queue("crm-followup", {
      connection: getRedisConnection(),
      defaultJobOptions: {
        attempts: 2,
...
var import_bullmq2, _redisAvailable2, _queue2, followUpQueue;
...
var import_axios2, emailService;
  ...
  "server/multitenant/services/ivrService.ts"() {
    ...
    ivrService = {
      async initiateCall(phone, appId) {
        ...
        const res = await import_axios4.default.post(url, params, {
...
function getCommunicationQueue() {
  if (!_queue3) {
    _queue3 = new import_bullmq3.Queue("crm-communication", {
      connection: getRedisConnection(),
      defaultJobOptions: {
        attempts: 3,
...
var import_bullmq3, _redisAvailable3, _queue3, communicationQueue;
...
function getSlaQueue() {
  if (!_queue4) {
    _queue4 = new import_bullmq4.Queue("crm-sla", { connection: getRedisConnection() });
  }
  return _queue4;
}
...
function getReEngagementQueue() {
  if (!_queue5) {
    _queue5 = new import_bullmq5.Queue("crm-reengagement", { connection: getRedisConnection() });
  }
  return _queue5;
}
...
var import_bullmq5, _queue5;
var init_reEngagementQueue = __esm({
  "server/multitenant/queues/reEngagementQueue.ts"() {
    "use strict";
    import_bullmq5 = require("bullmq");
    init_redisClient();
    _queue5 = null;
  }
});
...
function getInstallmentQueue() {
  if (!_queue6) {
    _queue6 = new import_bullmq6.Queue("crm-installments", { connection: getRedisConnection() });
  }
  return _queue6;
}
...
var import_bullmq6, _queue6;
var init_installmentQueue = __esm({
  "server/multitenant/queues/installmentQueue.ts"() {
    "use strict";
    import_bullmq6 = require("bullmq");
    init_redisClient();
    _queue6 = null;
  }
});
...
function startSlaWorker() {
  const worker = new import_bullmq9.Worker(
    "crm-sla",
    async (job) => {
...
function startFollowUpWorker() {
  const worker = new import_bullmq10.Worker(
    "crm-followup",
    async (job) => {
...
var init_followUpWorker = __esm({
  "server/multitenant/workers/followUpWorker.ts"() {
    "use strict";
    import_bullmq10 = require("bullmq");
    init_queues();
    init_dbPool();
    init_notificationService();
    init_whatsappService();
...
function startReEngagementWorker() {
  const worker = new import_bullmq11.Worker(
    "crm-reengagement",
    async (job) => {
      const { tenantId } = job.data;
      const db = await getPool(tenantId);
...
var init_reEngagementWorker = __esm({
  "server/multitenant/workers/reEngagementWorker.ts"() {
    "use strict";
    import_bullmq11 = require("bullmq");
    init_queues();
    init_dbPool();
    init_communicationService();
    init_logger();
...
function startInstallmentWorker() {
  const worker = new import_bullmq12.Worker(
    "crm-installments",
    async (job) => {
      const { tenantId } = job.data;
      const db = await getPool(tenantId);
...
var init_installmentWorker = __esm({
  "server/multitenant/workers/installmentWorker.ts"() {
    "use strict";
    import_bullmq12 = require("bullmq");
    init_queues();
    init_dbPool();
    init_whatsappService();
    init_logger();
...
function startNpsWorker() {
  const worker = new import_bullmq13.Worker(
    "crm-nps",
    async (job) => {
...
var init_npsWorker = __esm({
  "server/multitenant/workers/npsWorker.ts"() {
    "use strict";
    import_bullmq13 = require("bullmq");
    init_redisClient();
    init_dbPool();
    init_whatsappService();
    init_logger();
...
function startDigestWorker() {
  const worker = new import_bullmq14.Worker(
    "crm-digest",
    async (job) => {
...
var init_digestWorker = __esm({
  "server/multitenant/workers/digestWorker.ts"() {
    "use strict";
    import_bullmq14 = require("bullmq");
    init_redisClient();
    init_dbPool();
    init_whatsappService();
    init_logger();
...
function getNpsQueue() {
  if (!_queue7) {
    _queue7 = new import_bullmq15.Queue("crm-nps", { connection: getRedisConnection() });
  }
  return _queue7;
}
...
var import_bullmq15, _queue7;
var init_npsQueue = __esm({
  "server/multitenant/queues/npsQueue.ts"() {
    "use strict";
    import_bullmq15 = require("bullmq");
    init_redisClient();
    _queue7 = null;
  }
});
...
function getDigestQueue() {
  if (!_queue8) {
    _queue8 = new import_bullmq16.Queue("crm-digest", { connection: getRedisConnection() });
  }
  return _queue8;
}
...
var import_bullmq16, _queue8;
var init_digestQueue = __esm({
  "server/multitenant/queues/digestQueue.ts"() {
    "use strict";
    import_bullmq16 = require("bullmq");
    init_redisClient();
    _queue8 = null;
  }
});
...
init_env();
var import_meta = {};
var MIGRATIONS_DIR = import_path.default.resolve(
...
async function runMigrations(pool, tenantId) {
  ...
  try {
    ...
    `);
    const files = import_fs2.default.readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith(".sql")).sort();
    for (const file of files) {
      ...
      const sql = import_fs2.default.readFileSync(import_path.default.join(MIGRATIONS_DIR, file), "utf-8");
...
function signToken(payload) {
  return import_jsonwebtoken.default.sign(payload, env.JWT_SECRET, { expiresIn: EXPIRY });
}
...
var leadQuerySchema = import_zod2.z.object({
  stage: import_zod2.z.enum(["new", "contacted", "qualified", "demo", "interested", "payment", "admitted", "lost"]).optional(),
  source: import_zod2.z.enum(["meta_ads", "website", "manual", "excel_import", "walk_in", "phone", "referral"]).optional(),
  assignedTo: import_zod2.z.string().uuid().optional(),
  campaignId: import_zod2.z.string().uuid().optional(),
  ...
  limit: import_zod2.z.coerce.number().int().min(1).max(200).default(50)
});
var taskQuerySchema = import_zod2.z.object({
  assignedTo: import_zod2.z.string().uuid().optional(),
  status: import_zod2.z.enum(["pending", "in_progress", "done", "overdue", "cancelled"]).optional(),
  dueToday: import_zod2.z.coerce.boolean().optional(),
  overdue: import_zod2.z.coerce.boolean().optional(),
  page: import_zod2.z.coerce.number().int().min(1).default(1),
  limit: import_zod2.z.coerce.number().int().min(1).max(200).default(50)
});
var sendMessageSchema = import_zod2.z.object({
  leadId: import_zod2.z.string().uuid(),
  channel: import_zod2.z.enum(["whatsapp", "email", "sms", "ivr", "manual_call"]),
  templateId: import_zod2.z.string().uuid().optional(),
  subject: import_zod2.z.string().optional(),
  body: import_zod2.z.string().min(1),
  taskId: import_zod2.z.string().uuid().optional()
});
var reportExportSchema = import_zod2.z.object({
  type: import_zod2.z.enum(["funnel", "counsellor", "campaign_roi", "conversion", "all_leads"]),
  format: import_zod2.z.enum(["pdf", "excel"]),
  fromDate: import_zod2.z.string().optional(),
  toDate: import_zod2.z.string().optional(),
  counsellorId: import_zod2.z.string().uuid().optional(),
  campaignId: import_zod2.z.string().uuid().optional()
});
...
router.post("/login", authRateLimit, tenantFromHeader, async (req, res) => {
  ...
  if (!user || !await import_bcryptjs.default.compare(password, user.password_hash)) {
...
var leadSourceEnum = (0, import_pg_core.pgEnum)("lead_source", [
  "meta_ads",
  "website",
  "manual",
  "excel_import",
...
var leadStageEnum = (0, import_pg_core.pgEnum)("lead_stage", [
  "new",
  "contacted",
  "qualified",
  "demo",
...
var taskTypeEnum = (0, import_pg_core.pgEnum)("task_type", [
  "call",
  "whatsapp",
  "email",
  "sms",
...
var taskPriorityEnum = (0, import_pg_core.pgEnum)("task_priority", [
  "low",
  "medium",
  "high",
  "urgent"
]);
var channelEnum = (0, import_pg_core.pgEnum)("channel", [
  "whatsapp",
  "email",
  "sms",
  "ivr",
...
var commDirectionEnum = (0, import_pg_core.pgEnum)("comm_direction", [
  "outbound",
  "inbound"
]);
var commStatusEnum = (0, import_pg_core.pgEnum)("comm_status", [
  "queued",
  "sent",
  "delivered",
  "read",
...
var campaignSourceEnum = (0, import_pg_core.pgEnum)("campaign_source", [
  "meta_ads",
  "google_ads",
  "email",
  "sms",
...
var automationStatusEnum = (0, import_pg_core.pgEnum)("automation_exec_status", [
  "success",
  "failed",
  "skipped"
]);
var users = (0, import_pg_core.pgTable)("users", {
  ...
  username: (0, import_pg_core.varchar)("username", { length: 50 }).unique().notNull(),
...
var appSettings = (0, import_pg_core.pgTable)("app_settings", {
  ...
  key: (0, import_pg_core.varchar)("key", { length: 100 }).unique().notNull(),
...
var courses = (0, import_pg_core.pgTable)("courses", {
  ...
  fee: (0, import_pg_core.numeric)("fee", { precision: 10, scale: 2 }),
  isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true),
  createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
});
var campaigns = (0, import_pg_core.pgTable)("campaigns", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  name: (0, import_pg_core.varchar)("name", { length: 300 }).notNull(),
  source: campaignSourceEnum("source").notNull(),
  metaCampaignId: (0, import_pg_core.varchar)("meta_campaign_id", { length: 200 }),
  metaAdsetId: (0, import_pg_core.varchar)("meta_adset_id", { length: 200 }),
  startDate: (0, import_pg_core.date)("start_date"),
  endDate: (0, import_pg_core.date)("end_date"),
  budget: (0, import_pg_core.numeric)("budget", { precision: 12, scale: 2 }),
  isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true),
  createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
});
var campaignStats = (0, import_pg_core.pgTable)(
  "campaign_stats",
  {
    id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
    campaignId: (0, import_pg_core.uuid)("campaign_id").notNull().references(() => campaigns.id),
    date: (0, import_pg_core.date)("date").notNull(),
    leadsCount: (0, import_pg_core.integer)("leads_count").notNull().default(0),
    contactedCount: (0, import_pg_core.integer)("contacted_count").notNull().default(0),
    admittedCount: (0, import_pg_core.integer)("admitted_count").notNull().default(0),
    spend: (0, import_pg_core.numeric)("spend", { precision: 12, scale: 2 }),
    costPerLead: (0, import_pg_core.numeric)("cost_per_lead", { precision: 10, scale: 2 }),
    costPerAdmission: (0, import_pg_core.numeric)("cost_per_admission", {
      precision: 10,
      scale: 2
    }),
    updatedAt: (0, import_pg_core.timestamp)("updated_at", { withTimezone: true }).defaultNow()
  },
  (t) => [(0, import_pg_core.index)("idx_campaign_stats_campaign_date").on(t.campaignId, t.date)]
);
var leads = (0, import_pg_core.pgTable)(
  "leads",
  {
    id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
    leadNo: (0, import_pg_core.varchar)("lead_no", { length: 30 }).unique(),
    fullName: (0, import_pg_core.varchar)("full_name", { length: 200 }).notNull(),
    email: (0, import_pg_core.varchar)("email", { length: 150 }),
    phone: (0, import_pg_core.varchar)("phone", { length: 20 }).notNull(),
    alternatePhone: (0, import_pg_core.varchar)("alternate_phone", { length: 20 }),
    city: (0, import_pg_core.varchar)("city", { length: 100 }),
    qualification: (0, import_pg_core.varchar)("qualification", { length: 100 }),
    courseId: (0, import_pg_core.uuid)("course_id").references(() => courses.id),
    source: leadSourceEnum("source").notNull(),
    campaignId: (0, import_pg_core.uuid)("campaign_id").references(() => campaigns.id),
    adId: (0, import_pg_core.varchar)("ad_id", { length: 200 }),
    formId: (0, import_pg_core.varchar)("form_id", { length: 200 }),
    stage: leadStageEnum("stage").notNull().default("new"),
    subStage: (0, import_pg_core.varchar)("sub_stage", { length: 100 }),
    leadScore: (0, import_pg_core.integer)("lead_score").notNull().default(0),
    assignedTo: (0, import_pg_core.uuid)("assigned_to").references(() => users.id),
    isDuplicate: (0, import_pg_core.boolean)("is_duplicate").notNull().default(false),
    duplicateOf: (0, import_pg_core.uuid)("duplicate_of"),
    lostReason: (0, import_pg_core.varchar)("lost_reason", { length: 300 }),
    objectionNotes: (0, import_pg_core.text)("objection_notes"),
    reEngagementEligible: (0, import_pg_core.boolean)("re_engagement_eligible").notNull().default(true),
    lastContactedAt: (0, import_pg_core.timestamp)("last_contacted_at", { withTimezone: true }),
    admittedAt: (0, import_pg_core.timestamp)("admitted_at", { withTimezone: true }),
    createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow(),
    updatedAt: (0, import_pg_core.timestamp)("updated_at", { withTimezone: true }).defaultNow()
  },
  (t) => [
    (0, import_pg_core.index)("idx_leads_phone").on(t.phone),
    (0, import_pg_core.index)("idx_leads_email").on(t.email),
    (0, import_pg_core.index)("idx_leads_stage").on(t.stage),
    (0, import_pg_core.index)("idx_leads_assigned_to").on(t.assignedTo),
    (0, import_pg_core.index)("idx_leads_campaign_id").on(t.campaignId),
    (0, import_pg_core.index)("idx_leads_source").on(t.source),
    (0, import_pg_core.index)("idx_leads_created_at").on(t.createdAt)
  ]
);
var leadStageHistory = (0, import_pg_core.pgTable)("lead_stage_history", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
  fromStage: (0, import_pg_core.varchar)("from_stage", { length: 50 }),
  toStage: (0, import_pg_core.varchar)("to_stage", { length: 50 }).notNull(),
  changedBy: (0, import_pg_core.uuid)("changed_by").references(() => users.id),
  note: (0, import_pg_core.text)("note"),
  changedAt: (0, import_pg_core.timestamp)("changed_at", { withTimezone: true }).defaultNow()
});
var leadCustomFields = (0, import_pg_core.pgTable)("lead_custom_fields", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
  fieldKey: (0, import_pg_core.varchar)("field_key", { length: 100 }).notNull(),
  fieldValue: (0, import_pg_core.text)("field_value")
});
var opportunities = (0, import_pg_core.pgTable)("opportunities", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
  courseId: (0, import_pg_core.uuid)("course_id").references(() => courses.id),
  stage: (0, import_pg_core.varchar)("stage", { length: 50 }).notNull().default("new"),
  expectedFee: (0, import_pg_core.numeric)("expected_fee", { precision: 10, scale: 2 }),
  probability: (0, import_pg_core.integer)("probability").default(0),
  createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
});
var tasks = (0, import_pg_core.pgTable)(
  "tasks",
  {
    id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
    leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
    opportunityId: (0, import_pg_core.uuid)("opportunity_id").references(() => opportunities.id),
    assignedTo: (0, import_pg_core.uuid)("assigned_to").notNull().references(() => users.id),
    taskType: taskTypeEnum("task_type").notNull(),
    title: (0, import_pg_core.varchar)("title", { length: 300 }).notNull(),
    description: (0, import_pg_core.text)("description"),
    status: taskStatusEnum("status").notNull().default("pending"),
    priority: taskPriorityEnum("priority").notNull().default("medium"),
    dueAt: (0, import_pg_core.timestamp)("due_at", { withTimezone: true }).notNull(),
    completedAt: (0, import_pg_core.timestamp)("completed_at", { withTimezone: true }),
    reminderAt: (0, import_pg_core.timestamp)("reminder_at", { withTimezone: true }),
    outcome: (0, import_pg_core.text)("outcome"),
    createdBy: (0, import_pg_core.uuid)("created_by").references(() => users.id),
    createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow(),
    updatedAt: (0, import_pg_core.timestamp)("updated_at", { withTimezone: true }).defaultNow()
  },
  (t) => [
    (0, import_pg_core.index)("idx_tasks_lead_id").on(t.leadId),
    (0, import_pg_core.index)("idx_tasks_assigned_to").on(t.assignedTo),
    (0, import_pg_core.index)("idx_tasks_due_at").on(t.dueAt),
    (0, import_pg_core.index)("idx_tasks_status").on(t.status)
  ]
);
var slaPolicies = (0, import_pg_core.pgTable)("sla_policies", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  stage: (0, import_pg_core.varchar)("stage", { length: 50 }).notNull(),
  maxResponseHours: (0, import_pg_core.integer)("max_response_hours").notNull(),
  escalateTo: (0, import_pg_core.uuid)("escalate_to").references(() => users.id),
  isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true)
});
var slaTracking = (0, import_pg_core.pgTable)("sla_tracking", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
  taskId: (0, import_pg_core.uuid)("task_id").references(() => tasks.id),
  policyId: (0, import_pg_core.uuid)("policy_id").references(() => slaPolicies.id),
  breached: (0, import_pg_core.boolean)("breached").notNull().default(false),
  breachedAt: (0, import_pg_core.timestamp)("breached_at", { withTimezone: true }),
  escalationSent: (0, import_pg_core.boolean)("escalation_sent").notNull().default(false),
  createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
});
var messageTemplates = (0, import_pg_core.pgTable)("message_templates", {
  ...
  triggerEvent: (0, import_pg_core.varchar)("trigger_event", { length: 100 }),
  subject: (0, import_pg_core.varchar)("subject", { length: 500 }),
  body: (0, import_pg_core.text)("body").notNull(),
  variables: (0, import_pg_core.jsonb)("variables"),
  waTemplateName: (0, import_pg_core.varchar)("wa_template_name", { length: 200 }),
  isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true),
  createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
});
var communicationLogs = (0, import_pg_core.pgTable)(
  "communication_logs",
  {
    id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
    leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
    ...
    templateId: (0, import_pg_core.uuid)("template_id").references(() => messageTemplates.id),
    subject: (0, import_pg_core.varchar)("subject", { length: 500 }),
    body: (0, import_pg_core.text)("body").notNull(),
    sentBy: (0, import_pg_core.uuid)("sent_by").references(() => users.id),
    sentAt: (0, import_pg_core.timestamp)("sent_at", { withTimezone: true }),
    deliveredAt: (0, import_pg_core.timestamp)("delivered_at", { withTimezone: true }),
    readAt: (0, import_pg_core.timestamp)("read_at", { withTimezone: true }),
    externalMessageId: (0, import_pg_core.varchar)("external_message_id", { length: 300 }),
    errorMessage: (0, import_pg_core.text)("error_message"),
    createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
  },
  (t) => [
    (0, import_pg_core.index)("idx_comm_logs_lead_id").on(t.leadId),
    (0, import_pg_core.index)("idx_comm_logs_channel").on(t.channel),
    (0, import_pg_core.index)("idx_comm_logs_created_at").on(t.createdAt)
  ]
);
var automationRules = (0, import_pg_core.pgTable)("automation_rules", {
  ...
  name: (0, import_pg_core.varchar)("name", { length: 300 }).notNull(),
  description: (0, import_pg_core.text)("description"),
  triggerEvent: (0, import_pg_core.varchar)("trigger_event", { length: 100 }).notNull(),
  triggerConditions: (0, import_pg_core.jsonb)("trigger_conditions"),
  actions: (0, import_pg_core.jsonb)("actions").notNull(),
  delayMinutes: (0, import_pg_core.integer)("delay_minutes").notNull().default(0),
  isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true),
  executionCount: (0, import_pg_core.integer)("execution_count").notNull().default(0),
  createdBy: (0, import_pg_core.uuid)("created_by").references(() => users.id),
  createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
});
var automationExecutionLog = (0, import_pg_core.pgTable)("automation_execution_log", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  ruleId: (0, import_pg_core.uuid)("rule_id").notNull().references(() => automationRules.id),
  leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
  triggeredAt: (0, import_pg_core.timestamp)("triggered_at", { withTimezone: true }).defaultNow(),
  actionsTaken: (0, import_pg_core.jsonb)("actions_taken"),
  status: automationStatusEnum("status").notNull(),
  errorMessage: (0, import_pg_core.text)("error_message")
});
var reEngagementCampaigns = (0, import_pg_core.pgTable)("reengagement_campaigns", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  name: (0, import_pg_core.varchar)("name", { length: 300 }).notNull(),
  description: (0, import_pg_core.text)("description"),
  targetStage: (0, import_pg_core.varchar)("target_stage", { length: 50 }),
  dormantDays: (0, import_pg_core.integer)("dormant_days").notNull(),
  channel: channelEnum("channel").notNull(),
  templateId: (0, import_pg_core.uuid)("template_id").references(() => messageTemplates.id),
...
var reEngagementLog = (0, import_pg_core.pgTable)("reengagement_log", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  campaignId: (0, import_pg_core.uuid)("campaign_id").notNull().references(() => reEngagementCampaigns.id),
  leadId: (0, import_pg_core.uuid)("lead_id").notNull().references(() => leads.id),
  attemptNumber: (0, import_pg_core.integer)("attempt_number").notNull(),
  sentAt: (0, import_pg_core.timestamp)("sent_at", { withTimezone: true }).defaultNow(),
  responseReceived: (0, import_pg_core.boolean)("response_received").notNull().default(false)
});
var reportTemplates = (0, import_pg_core.pgTable)("report_templates", {
  id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
  reportKey: (0, import_pg_core.varchar)("report_key", { length: 100 }).unique().notNull(),
  htmlTemplate: (0, import_pg_core.text)("html_template").notNull(),
  css: (0, import_pg_core.text)("css"),
  isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true),
  updatedAt: (0, import_pg_core.timestamp)("updated_at", { withTimezone: true }).defaultNow()
});
var notifications = (0, import_pg_core.pgTable)(
  "notifications",
  {
    ...
    type: (0, import_pg_core.varchar)("type", { length: 80 }).notNull(),
    title: (0, import_pg_core.varchar)("title", { length: 300 }).notNull(),
    body: (0, import_pg_core.text)("body"),
    leadId: (0, import_pg_core.uuid)("lead_id").references(() => leads.id),
    taskId: (0, import_pg_core.uuid)("task_id").references(() => tasks.id),
    isRead: (0, import_pg_core.boolean)("is_read").notNull().default(false),
    createdAt: (0, import_pg_core.timestamp)("created_at", { withTimezone: true }).defaultNow()
  },
  (t) => [
    (0, import_pg_core.index)("idx_notifications_user").on(t.userId, t.isRead, t.createdAt)
  ]
);
...
var insertLeadSchema = (0, import_drizzle_zod.createInsertSchema)(leads, {
  phone: import_zod3.z.string().min(10).max(15),
  email: import_zod3.z.string().email().optional().or(import_zod3.z.literal("")),
  leadScore: import_zod3.z.number().min(0).max(100).optional()
...
router8.patch("/users/:id", ...adminGuard, async (req, res) => {
  ...
  if (password) {
    ...
    params.push(await import_bcryptjs2.default.hash(password, 12));
  }
  if (!sets.length) {
    res.json({ ok: true });
    return;
...
var razorpayService = {
  ...
  async createPaymentLink(opts) {
    ...
    const res = await import_axios5.default.post(`${BASE2}/payment_links`, body, { headers: authHeader() });
  ...
  verifyWebhookSignature(rawBody, signature) {
    ...
    const expected = import_crypto.default.createHmac("sha256", env.RAZORPAY_WEBHOOK_SECRET).update(rawBody).digest("hex");
...
router9.post("/meta", async (req, res) => {
  ...
  if (env.META_APP_SECRET && signature) {
    ...
    if (rawBody) {
      const expected = `sha256=${import_crypto3.default.createHmac("sha256", env.META_APP_SECRET).update(rawBody).digest("hex")}`;
      if (!import_crypto3.default.timingSafeEqual(Buffer.from(signature), Buffer.from(expected))) {
...
async function fetchMetaLeadData(leadgenId) {
  const res = await import_axios6.default.get(
...
router14.post("/whatsapp-send", async (req, res) => {
  ...
  const metaRes = await import_axios7.default.post(url, {
    messaging_product: "whatsapp",
    to,
    type: "template",
...
router15.post("/messages/send", async (req, res) => {
  ...
  let errorMsg;
  try {
    const axiosRes = await import_axios8.default.post(url, requestPayload, {
      headers: {
        Authorization: `Bearer ${env.META_ACCESS_TOKEN}`,
        "Content-Type": "application/json"
      },
...
function createApp() {
  ...
  app.use(
    "/api/webhooks/meta",
    import_express27.default.raw({ type: "application/json" }),
    (req, _res, next) => {
      req.rawBody = req.body;
      next();
    }
  );
  app.use("/api/webhooks/razorpay", import_express27.default.json());
  ...
  if (env.NODE_ENV === "production") {
    import("path").then(({ default: path3 }) => {
      import("url").then(({ fileURLToPath: fileURLToPath2 }) => {
        const __dirname = path3.dirname(fileURLToPath2(import_meta3.url));
        app.use(import_express27.default.static(path3.join(__dirname, "../public")));
        app.get("*", (_req, res) => {
